...
class VAE(tf.keras.Model):
    ...
    def build_vae(self):
        ...
        opt = optimizers.legacy.Adam(learning_rate=self.learning_rate)
    ...
    def best_predict_anomaly(self, train, test,labels,threshold_per_list=None):
        mae_train = self.calculate_reconstruction_error(train)
        mae_data = self.calculate_reconstruction_error(test)
        if threshold_per_list is None:
            threshold_per_list= np.arange(0.9,1+0.0001,0.0001)
        best_f1 = 0
        best_pred = None
        for idx, threshold_per in enumerate(threshold_per_list):
            threshold = np.quantile(mae_train.values, threshold_per)
            pred = [1 if curr_mae > threshold else 0 for curr_mae in mae_data]
            f1 = f1_score(labels, pred,average='macro')
            if f1 >= best_f1:
                best_f1 = f1
                self.threshold_per = threshold_per
                best_pred = pred
        return best_pred, mae_data, mae_train
