...
class DataPipeline():
    ...
    def load_HPC_data(self, data_dir):
        ...
        if not self.x_train_filename and not self.y_train_filename:
            assert list(x_train.index.get_level_values('instance').unique()) == list(y_train.index.unique())        
        ...
        if not self.x_val_filename and not self.y_val_filename:
            assert list(x_val.index.get_level_values('instance').unique()) == list(y_val.index.unique())        
    ...
    def tsfresh_generate_features(self, data, fe_config, kind_to_fc_parameters=None, column_id="uid", column_sort="timestamp"):
        ...
        if data.index.names == ['instance', 'timestamp']:
            data.reset_index(inplace=True)
         
        #Create a unique id column since job_id and instance combo is the unique one
        data['uid'] = data['job_id'].astype(str) + '_' + data['instance'].astype(str)
        data.drop(columns=['job_id','instance'],inplace=True)
        # elif column_id == "instance":
        #     continue
        ...
        data_fe[['job_id', 'instance']] = data_fe['index'].str.split('_', expand=True)
        data_fe.drop(columns=['index'], inplace=True)
    
        if self.window_size == 0 :
            data_fe.set_index(["job_id", "instance"],inplace=True)
        else:
            data_fe.set_index(["job_id", "instance", "timestamp"],inplace=True)
    ...
    def _read_label(self, abs_input_path):
        ...
        try:
            ...
            label['instance'] = label['instance'].astype('str') 
            label.set_index(['job_id', 'instance'], inplace=True)
