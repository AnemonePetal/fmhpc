...
def thresholding_edges(table, edges):
   table = table.copy()
   table = table.sort_values(by = "score", ascending = False)
   table = table.iloc[:edges]
   return table
...
def maximum_spanning_tree(table, undirected = False,return_graph = True):
   ...
   if return_graph:
      ...
      table2 = nx.to_pandas_edgelist(T).reset_index()
...
def maximum_Edmonds_tree(table, undirected = False,return_graph = True):
   sys.stderr.write("Calculating MET score...\n")
   table = table.copy()
   table["distance"] = 1.0 / table["nij"]
   G = nx.from_pandas_edgelist(table, source = "src", target = "trg", edge_attr = ["distance", "nij"], create_using=nx.DiGraph())
   T = nx.maximum_spanning_arborescence(G, attr='distance')
   if return_graph:
      return T
   else:
      table2 = nx.to_pandas_edgelist(T).reset_index()
